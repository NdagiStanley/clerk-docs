...
<Steps>
  ...
  Clerk's [Express SDK](/docs/backend-requests/session-requests/handling/nodejs) gives you access to prebuilt components, React hooks, and helpers to make user authentication easier.
  ...
  To protect your routes, use the [`requireAuth()`](/docs/backend-requests/session-requests/handling/nodejs#require-auth) middleware. This middleware functions similarly to `clerkMiddleware()`, but also protects your routes by redirecting unauthenticated users to the sign-in page.

  In the following example, `requireAuth()` is used to protect the `/protected` route. If the user isn't authenticated, they're redirected to the homepage. If the user is authenticated, the [`getAuth()`](/docs/backend-requests/session-requests/handling/nodejs#get-auth) function is used to get the `userId`, which is passed to [`clerkClient.users.getUser()`](/docs/references/backend/user/get-user){{ target: '_blank' }} to fetch the current user's `User` object.
...
<Cards>
  - [Use middleware to protect routes](/docs/backend-requests/session-requests/handling/nodejs#require-auth)
  - Learn how to protect specific routes from unauthenticated users.

  ---

  - [Protect routes based on authorization status](/docs/backend-requests/session-requests/handling/nodejs#get-auth)
  - Learn how to protect a route based on both authentication and authorization status.

  ---

  - [Express SDK reference](/docs/backend-requests/session-requests/handling/nodejs)
