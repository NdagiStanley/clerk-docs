...
The `auth()` helper needs to know if it should authenticate a [session request](/docs/backend-requests/session-requests/overview) or a [machine request](/docs/machine-requests/overview).
